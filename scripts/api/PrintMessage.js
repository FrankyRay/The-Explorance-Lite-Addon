...
export function RolesPrint(message, roles, playerName, target = "@a") {
  switch (roles) {
    case 'admin':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§g[ADMIN] ${playerName}§r: ${message}"}]}`);
      break;
    case 'mods':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§9[MODS] ${playerName}§r: ${message}"}]}`);
      break;
    case 'red_team':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§c[RED TEAM] ${playerName}§r: ${message}"}]}`);
      break;
    case 'yellow_team':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§e[RED TEAM] ${playerName}§r: ${message}"}]}`);
      break;
    case 'green_team':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§a[RED TEAM] ${playerName}§r: ${message}"}]}`);
      break;
    case 'blue_team':
      Overworld.runCommand(`tellraw ${target} {"rawtext": [{"text": "§b[RED TEAM] ${playerName}§r: ${message}"}]}`);
      break;
  }
}
