...
function coordinate() {
  world.events.tick.subscribe(tick => {
    for (let player of world.getPlayers()) {
      ...
      try {
        Overworld.runCommand(`scoreboard players set "${player.name}" X_Coord ${x}`)
        Overworld.runCommand(`scoreboard players set "${player.name}" Y_Coord ${y}`)
        Overworld.runCommand(`scoreboard players set "${player.name}" Z_Coord ${z}`) 
        Overworld.runCommand(`scoreboard players set "${player.name}" Health ${health}`) 
      } catch (err) {}
    }
  })
}
