...
function SetHome() {
  world.events.beforeChat.subscribe((eventData) => {
    if (eventData.message === "!sethome") {
      ...
      print(
        `Your home was set on ${coord}. Type '!tphome' to teleport to your home point`
      );
      ...
      player.getTags().forEach((tag) => {
        ...
        if (!coord) {
          print(
            "Oh no! You didn't have home point. Type '!tphome' to set your home"
          );
        } else {
          world
            .getDimension("overworld")
            .runCommand(`tp ${player.name} ${coord}`);
        }
      });
      eventData.cancel = true;
    }
